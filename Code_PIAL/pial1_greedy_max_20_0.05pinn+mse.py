...
timing = time.strftime('%m%d-%H%M%S', time.localtime()) + "-greeedy-max-20-0.1pinnloss"
...
class Trainer():
    ...
    def train(self, iterations: int, indices: list[int]):
        ...
        for step in pbar:
            ...
            self.opt.zero_grad()
            loss = loss_mse + loss_pde * 0.05
            loss.backward()
            grad = torch.nn.utils.clip_grad_norm_(self.net.parameters(), np.inf)
            self.opt.step()
            self.lr_scheduler.step()
            if self.global_step % 200 == 0:
                pbar.set_postfix(loss_mse = loss_mse.item(), loss_pde = 0.05 * loss_pde.item(), grad = grad.item())
