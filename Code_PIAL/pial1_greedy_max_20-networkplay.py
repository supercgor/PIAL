...
timing = time.strftime('%m%d-%H%M%S', time.localtime()) + "-greeedy-max-20-networkplay"

os.makedirs(f"model/{timing}/runs", exist_ok = True)
logging.basicConfig(filename = f"model/{timing}/train.log", level = logging.INFO)
...
class customDeepONet(nn.Module):
    def __init__(self):
        ...
        self.output_transform = dirichlet
        
    def forward(self, inputs):
        vx, grid = inputs
        branch = self.branch(vx)
        trunk = self.trunk(grid)
        outputs = self.b + torch.sum(branch * trunk, dim = 1, keepdim = True)
        return self.output_transform(inputs, outputs)

class Trainer():
    def __init__(self):
        self.net = customDeepONet()
        ...
        self.global_step = 0
        self.tb_writer = SummaryWriter(f"model/{timing}/runs")
    ...
    def train(self, iterations: int, indices: list[int]):
        ...
        data_sampler = BatchSampler(RandomSampler(data, generator = torch.Generator(torch.device("cuda"))), batchsize, True)
    ...
    def test(self, indices: list[int] = None, 
        ...
        gt = UXT[indices]
        loss_mse = F.mse_loss(pred, gt, reduction="none").mean(dim = 1)
...
if __name__ == "__main__":
    ...
    while len(indices) < n_2 and i < train_vx.shape[0]:
        ...
        ter.tb_writer.add_scalars("training/pde residual", {str(i) : key for i, key in enumerate(pde_res)}, ter.global_step)
        ter.tb_writer.add_scalars("training/l2 relative error", {str(i) : key for i, key in enumerate(l2_rel)}, ter.global_step)
        ter.tb_writer.add_scalars("training/mse loss", {str(i) : key for i, key in enumerate(test_mse)}, ter.global_step)
        
        if len(indices) == 0: # pretrain
            indices = list(range(n_init))
            i += n_init
        else: # train0
            select_indices = torch.topk(pde_res, k = min(n_1, n_2 - len(indices)))[1] + i
            i += n_0
        ...
        ter.tb_writer.add_scalar("testing/steps", data_len, ter.global_step)
        ter.tb_writer.add_scalars("testing/mse loss", {str(i) : key for i, key in enumerate(test_mse)}, ter.global_step)
        ter.tb_writer.add_scalars("testing/pde residual", {str(i) : key for i, key in enumerate(pde_res)}, ter.global_step)
        ter.tb_writer.add_scalars("testing/l2 relative error", {str(i) : key for i, key in enumerate(l2_rel)}, ter.global_step)

        logging.info(f"{len(indices)} data: mse: {test_mse.mean().item():.2e}, rel_err: {l2_rel.mean().item():.2e}, pde: {pde_res.mean().item():.2e}")
